...
class FirestoreHelper {
  ...
  static final String KEY_BOOKS = 'Books';
  static final String KEY_PDFS = 'Pdfs';
 ...
 //   Common.signedInUser = user;
  ...
  static void createNewBook(User user,  Book book, String bookImgURL, Function onCompleteCallback) async {
    book.bookURL = bookImgURL;
    Firestore firestore = Firestore.instance;

    CollectionReference BOOKS_COLLECTION = firestore.collection(KEY_BOOKS);

    DocumentReference Book = BOOKS_COLLECTION.document(DateTime.now().toString());

    Book.setData(book.toMap()).whenComplete(() => onCompleteCallback.call());
    //   Common.signedInUser = user;
  }
  ...
  static void createNewPDF(User user,  PDFs book, Function onCompleteCallback) async {
    ...
    DocumentReference pdf = PDFS_COLLECTION.document(DateTime.now().toString());

    pdf.setData(book.toMap()).whenComplete(() => onCompleteCallback.call());
    //   Common.signedInUser = user;
  }
